...
def zaoan():
        if (Whether_zaoan != False):
            try:
                ...
                params = urllib.parse.urlencode({'key': '29fa92a4cb9af24c4152f8417f302f71'})
...
def tip():
    if (Whether_tip != False):
        try:
            ...
            # params = urllib.parse.urlencode({'key': '29fa92a4cb9af24c4152f8417f302f71', 'city': '101020100', 'type': '1'})
            # headers = {'Content-type': 'application/x-www-form-urlencoded'}
            # conn.request('POST', '/tianqi/index', params, headers)
            # tianapi = conn.getresponse()
            # data = tianapi.read()
...
# 推送信息
def send_message(to_user, access_token, city_name, weather, max_temperature, min_temperature, pipi, lizhi, pop, tips,zaoan,
...
if __name__ == "__main__":
    try:
        ...
        print("推送消息失败，请检查config1.txt文件是否与程序位于同一路径")
        os.system("pause")
        sys.exit(1)
    except SyntaxError:
    ...
    # 公众号推送消息
    for user in users:
        send_message(user, accessToken, city, weather, max_temperature, min_temperature, pipi, lizhi,
                      pop,tips,zaoan,
                     note_en, note_ch, health_tip, lucky_)
    import time
